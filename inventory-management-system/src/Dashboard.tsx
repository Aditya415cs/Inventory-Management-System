...
export default function Dashboard() {
    ...
    const load = async () => {
      try {
        ...
        setStats({
          ...
          lowStock: products.filter((p) => p.quantity < 20 && p.quantity > 0)
            .length,
        });
        // ML API call
        ...
        const formatted = (mlJson.graph_data || []).map((item: any) => ({
          name: item.day,
          value: item.sales,
        }));

        setMlData(formatted);
        setPrediction(mlJson.prediction);
      } catch (e) {
        console.error(e);
      } finally {
        setLoading(false);
  ...
  const last7 = Array.from({ length: 7 }, (_, i) => {
    ...
    const count = orders.filter(
      (o) => o.type === "sale" && o.created_at?.slice(0, 10) === key,
    ).length;
  ...
  const pipeline = [
    {
      name: "Confirmed",
      value: orders.filter((o) => o.type === "sale" && o.status === "confirmed")
        .length,
    },
    {
      name: "Packing",
      value: orders.filter((o) => o.type === "sale" && o.status === "packing")
        .length,
    },
    {
      name: "Dispatched",
      value: orders.filter(
        (o) => o.type === "sale" && o.status === "dispatched",
      ).length,
    },
  ];
  ...
  return (
    <div className="main">
      ...
      {loading ? (
        <p style={{ color: "#94a3b8" }}>Loading live data…</p>
      ) : (
        <>
          {/* KPI Cards */}
          <div className="cards">
            ...
            <div
              className="card"
              onClick={() => navigate("/app/order-processing")}
              style={{ cursor: "pointer" }}
            >
              <h3>Sales Orders</h3>
              <p>{stats.sales}</p>
            ...
            <div
              className="card"
              onClick={() => navigate("/app/manufacturing")}
              style={{ cursor: "pointer" }}
            >
              <h3>Mfg Batches</h3>
              <p>{stats.batches}</p>
          ...
          <div className="charts">
            ...
            <div className="chart-box">
              <h2>ML Sales Prediction (Last 7 Days)</h2>

              <h3 style={{ color: "#22c55e" }}>
                Predicted Next Sales: {prediction}
              </h3>
